...
<html>
	...
	<body>
		...
		<div class="container">
			...
			<?php
					$l_sqlstr = "select * from tb_post where type1_id = '2'";//定义查询语句
					...
					$l_sqlstr = "select tb_post.id,tb_member.id,head,name,title,content,now,discussnum,member_id,type1_id from tb_post,tb_member where type1_id='2' and tb_member.id=member_id order by tb_post.id desc limit ".($page-1)*$pagesize.",".$pagesize;//定义查询语句,按照评论数降序排列
					?>
				<?php
				 	$result = $pdo->prepare($l_sqlstr);//准备查询
				 	$result->execute();//执行查询
			...
			<div class="postmain">
				<table width="100%" cellpadding="0" cellspacing="0" width="100%" style="border-collapse: collapse;">
					<tr bgcolor="#DDEFFD" style="font-size:16px;">
						<td width="120px" height="60px" align="center" valign="middle">生活区</td>
